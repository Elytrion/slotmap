...
namespace sparse_slotmap
{
    struct sparse_key_domain {};

    template <typename T>
    using slot_map_key64 = slotmap_commons::slot_map_key64<T, sparse_key_domain>;

    template <typename T>
    using slot_map_key32 = slotmap_commons::slot_map_key32<T, sparse_key_domain>;
    ...
    */
    template <
        typename T,
        typename TKeyType = slot_map_key64<T>,
    ...
    using slot_map32 = slot_map<T, sparse_slotmap::slot_map_key32<T>, PAGESIZE, MINFREEINDICES>;

    template <class T, size_t PAGESIZE = 4096, size_t MINFREEINDICES = 64>
    using slot_map64 = slot_map<T, sparse_slotmap::slot_map_key64<T>, PAGESIZE, MINFREEINDICES>;
...
namespace std
{
    template <typename T> struct hash<typename sparse_slotmap::slot_map_key64<T>>
    {
        size_t operator()(const typename sparse_slotmap::slot_map_key64<T>& key) const noexcept { return key.hash(); }
    };

    template <typename T> struct hash<typename sparse_slotmap::slot_map_key32<T>>
    {
        size_t operator()(const typename sparse_slotmap::slot_map_key32<T>& key) const noexcept { return key.hash(); }
    };
